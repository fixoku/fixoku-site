...
try {
  ...
  assert.deepEqual(tableResult.rows.map((row) => row.tablename), ["account", "platform_audit_events", "platform_memberships", "platform_users", "rate_limit", "session", "user", "verification"]);
  ...
  const foreignKeyResult = await client.query("select count(*)::int as count from information_schema.table_constraints where table_schema = 'public' and table_name in ('platform_memberships','platform_audit_events') and constraint_type = 'FOREIGN KEY'");
  assert.equal(foreignKeyResult.rows[0].count, 2);
