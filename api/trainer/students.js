...
export default async function handler(req, res) {
  ...
  try {
    ...
    try {
      ...
      const students = rows.map(({ user, profile, assignment }) => ({ id: user.id, name: user.displayName, email: user.email, grade: profile?.grade ?? null, school: profile?.school ?? null, status: profile?.status ?? "ACTIVE", linkedAt: assignment.createdAt }));
