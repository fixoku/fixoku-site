...
async function signIn(page: any, email: string) { await page.goto("/giris"); await handleConsent(page); await page.locator("#login-email").fill(email); await page.locator("#login-password").fill(password); await page.locator("form button[type=submit]").click(); await page.waitForURL("**/panel/ogrenci"); }
