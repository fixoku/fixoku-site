...
export default async function handler(request, response) {
  ...
  try {
    ...
    const requestedAllowed = ["/panel/egitmen/profil", "/panel/egitmen/egitimlerim", "/panel/egitmen/sunumlarim", "/panel/egitmen/kaynaklarim", "/panel/egitmen/musaitlik", "/panel/egitmen/bakiyem", "/panel/egitmen/ogrencilerim"].includes(requestedPanel) || requestedPanel?.startsWith("/panel/egitmen/ogrencilerim/") ? context.roles.includes("TRAINER") : (context.destinations?.includes(requestedPanel) || context.destination === requestedPanel);
