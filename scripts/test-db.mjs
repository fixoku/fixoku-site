...
try {
  ...
  assert.deepEqual(tableResult.rows.map((row) => row.tablename), ["account", "availability_slots", "enrollments", "guardian_profiles", "guardian_student_relationships", "platform_audit_events", "platform_memberships", "platform_users", "presentations", "rate_limit", "resources", "session", "student_profiles", "trainer_assignments", "trainer_earning_ledger", "trainer_entitlements", "trainer_profiles", "trainer_student_links", "training_programs", "user", "verification"]);
  ...
  const foreignKeyResult = await client.query("select count(*)::int as count from information_schema.table_constraints where table_schema = 'public' and table_name in ('platform_memberships','platform_audit_events','trainer_profiles','trainer_entitlements','training_programs','presentations','resources','student_profiles','trainer_student_links','enrollments','trainer_assignments','guardian_profiles','guardian_student_relationships','availability_slots','trainer_earning_ledger') and constraint_type = 'FOREIGN KEY'");
  assert.equal(foreignKeyResult.rows[0].count, 21);
