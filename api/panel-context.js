...
export default async function handler(request, response) {
  ...
  try {
    ...
    const requestedAllowed = ["/panel/egitmen/profil", "/panel/egitmen/egitimlerim", "/panel/egitmen/sunumlarim", "/panel/egitmen/kaynaklarim", "/panel/egitmen/musaitlik", "/panel/egitmen/bakiyem", "/panel/egitmen/payout-hesabim", "/panel/egitmen/ogrencilerim"].includes(requestedPanel) || requestedPanel?.startsWith("/panel/egitmen/ogrencilerim/") ? context.roles.includes("TRAINER") : ["/panel/admin", "/panel/admin/atamalar", "/panel/admin/paketler", "/panel/admin/egitmenler", "/panel/admin/kargo", "/panel/admin/urunler", "/panel/admin/saglayicilar"].includes(requestedPanel) || requestedPanel?.startsWith("/panel/admin/paketler/") || requestedPanel?.startsWith("/panel/admin/egitmenler/") || requestedPanel?.startsWith("/panel/admin/urunler/") ? context.roles.some((role) => ["OWNER", "SUPER_ADMIN"].includes(role)) : requestedPanel?.startsWith("/panel/owner") ? context.roles.includes("OWNER") : ["/panel/ogrenci", "/panel/ogrenci/profil", "/panel/ogrenci/paketler", "/panel/ogrenci/kargolar"].includes(requestedPanel) ? context.roles.some((role) => ["STUDENT", "GUARDIAN"].includes(role)) : ["/panel/bildirimler", "/panel/notifications"].includes(requestedPanel) ? Boolean(context.roles.length) : (context.destinations?.includes(requestedPanel) || context.destination === requestedPanel);
    ...
    const requiredPermission = ["OWNER", "SUPER_ADMIN"].includes(role) ? "user.manage" : role === "TRAINER" ? "trainer.read" : "student.read";
